...
$xml =  simplexml_load_file("historial_del_personal.jrxml");
...
$PHPJasperXML->arrayParameter=array("cedula"=>"'$cedula'","nro_registro"=>"null");
...
if($PHPJasperXML->query_status==1){
	...
	$Auditoria_Proceso->parametro_valor("{cedula: \'".$cedula."\'}");
	...
	if($Auditoria_Proceso->Obtener_Nro_Registro()){
		...
		$PHPJasperXML->arrayParameter=array("cedula"=>"'$cedula'","nro_registro"=>"'$nro_registro");
	}
	else
		die('Error: '.$Auditoria_Proceso->error());
}
