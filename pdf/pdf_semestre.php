<?php
  ...
  $this->Cell(0,6,"UNIVERSIDAD DEPORTIVA DEL SUR",0,1,"C");
    $this->Cell(0,6,"\"Sistema de Ingreso Estudiantes y Horarios\"",0,1,"C");
       $this->Ln(15);  
   $this->Cell(0,6,'LISTADO DE LOS SEMESTRES',0,1,"C");
   $this->Ln(5);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
                  ...
                  $this->Cell($anchura*4,$altura,utf8_decode('AÑO ACADÉMICO'),1,0,'L',$color_fondo);
...
function NbLines($w,$txt)
{
  ...
  $mysql=new Conexion();
  $sql="select *,
   (CASE WHEN tano_academico.fecha_desactivacion IS NULL THEN  'Activo' 
  ELSE 'Desactivado' END) AS estatus from tsemestre 
inner join tano_academico on tano_academico.cod_ano_academico=tsemestre.cod_ano_academico";
  $i=-1;
  $data=$mysql->Ejecutar($sql);
    if($mysql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['cod_semestre'])))),
         utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['semestre'])))),
