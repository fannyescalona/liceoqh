...
function NbLines($w,$txt)
{
 ...
 $this->Image("../images/UDS.jpg" , 10 ,15, 40 , 40, "JPG" ,$_SERVER['HTTP_HOST']."/udesur/vistas");
    ...
    //Título
		    $this->Cell(10);
			$this->SetFont('Arial','B',10);
			$this->Cell(0,7,"PROFESOR(A): ".strtoupper($_SESSION['datos']['nombre'][0]." ".$_SESSION['datos']['apellido'][0]),0,1,"L");    //Salto de línea
		    $this->Cell(10);		
			$this->Cell(0,7,"CÉDULA: ".strtoupper($_SESSION['datos']['cedula'][0]),0,1,"L"); 
...
$id_hora=$_SESSION['datos']['id_hora'];
    ...
    for($i=0;$i<count($get_hora['id']);$i++){	
	 ...
	 // $lobjPdf->Cell($anchura-7,$altura,utf8_decode($hora_nombre),1,0,$alignacion);
	   ...
	   (in_array($x."-1",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Sección:'.$_SESSION['datos']['seccion'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';

	     $valor_a_buscar[0]=$datos_celda;
	   if(in_array($x."-2",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-1",$_SESSION['datos']['celda']);}	  
	   (in_array($x."-2",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Sección:'.$_SESSION['datos']['seccion'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';

	     $valor_a_buscar[1]=$datos_celda;
	   if(in_array($x."-3",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-1",$_SESSION['datos']['celda']);}
      (in_array($x."-3",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Sección:'.$_SESSION['datos']['seccion'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';

	  	$valor_a_buscar[2]=$datos_celda;
	   if(in_array($x."-4",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-1",$_SESSION['datos']['celda']);}	 
	  (in_array($x."-4",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Sección:'.$_SESSION['datos']['seccion'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';

	  	     $valor_a_buscar[3]=$datos_celda;
	   if(in_array($x."-5",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-1",$_SESSION['datos']['celda']);}
	  (in_array($x."-5",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Sección:'.$_SESSION['datos']['seccion'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';
	   ...
	   if(in_array($x."-6",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-1",$_SESSION['datos']['celda']);}
	   	(in_array($x."-6",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Sección:'.$_SESSION['datos']['seccion'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda='-';

	  	     $valor_a_buscar[5]=$datos_celda;
	   if(in_array($x."-0",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-1",$_SESSION['datos']['celda']);}
	  (in_array($x."-0",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Sección:'.$_SESSION['datos']['seccion'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda='-';
