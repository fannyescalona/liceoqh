...
function NbLines($w,$txt)
{
 ...
 $this->Image("../images/UDS.jpg" , 10 ,15, 40 , 40, "JPG" ,$_SERVER['HTTP_HOST']."/udesur/vistas");
      ...
      $this->Cell(0,6,"REPÚBLICA BOLIVARIANA DE VENEZUELA",0,1,"C");
     $this->Cell(0,6,"MINISTERIO DEL PODER POPULAR PARA LA EDUCACIÓN SUPERIOR",0,1,"C");
                 $this->SetFont('Arial','B',10);  
     $this->Cell(0,6,"UNIVERSIDAD DEPORTIVA DEL SUR",0,1,"C");
                 $this->SetFont('Arial','',10); 
             $this->Cell(0,6,"SECRETARÍA GENERAL",0,1,"C");
             $this->SetFont('Arial','B',10);  
                 $this->Cell(0,6,"DIRECCIÓN DE CONTROL DE ESTUDIOS",0,1,"C");
      $this->Ln(8); 
    //Título
			$this->SetFont('Arial','B',14);
			$this->Cell(0,7,"* HORARIO DE LA SECCIÓN ".$_POST['seccion']." *",0,1,"C");  
			$this->SetFont('Arial','B',10);
			//$this->Cell(67,7,"LAPSO ACADEMICO: ".substr($_SESSION['datos']['lapso'],0,4).'-'.substr($_SESSION['datos']['lapso'],4,7),0,1,"C");    //Salto de línea
		    $this->Ln(8);			
...
$id_hora=$_SESSION['datos']['id_hora'];
    ...
    for($i=0;$i<count($get_hora['id']);$i++){	
	 ...
	 // $lobjPdf->Cell($anchura-7,$altura,utf8_decode($hora_nombre),1,0,$alignacion);
	   ...
	   (in_array($x."-1",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Prof:'.$_SESSION['datos']['nombre'][$valor].' '.$_SESSION['datos']['apellido'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';

	     $valor_a_buscar[0]=$datos_celda;
	   if(in_array($x."-2",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-2",$_SESSION['datos']['celda']);}	  
	   (in_array($x."-2",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Prof:'.$_SESSION['datos']['nombre'][$valor].' '.$_SESSION['datos']['apellido'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';

	     $valor_a_buscar[1]=$datos_celda;
	   if(in_array($x."-3",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-3",$_SESSION['datos']['celda']);}
      (in_array($x."-3",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Prof:'.$_SESSION['datos']['nombre'][$valor].' '.$_SESSION['datos']['apellido'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';

	  	$valor_a_buscar[2]=$datos_celda;
	   if(in_array($x."-4",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-4",$_SESSION['datos']['celda']);}	 
	  (in_array($x."-4",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Prof:'.$_SESSION['datos']['nombre'][$valor].' '.$_SESSION['datos']['apellido'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';

	  	     $valor_a_buscar[3]=$datos_celda;
	   if(in_array($x."-5",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-5",$_SESSION['datos']['celda']);}
	  (in_array($x."-5",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Prof:'.$_SESSION['datos']['nombre'][$valor].' '.$_SESSION['datos']['apellido'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda=' ';
	   ...
	   if(in_array($x."-6",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-6",$_SESSION['datos']['celda']);}
	   	(in_array($x."-6",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Prof:'.$_SESSION['datos']['nombre'][$valor].' '.$_SESSION['datos']['apellido'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda='-';

	  	     $valor_a_buscar[5]=$datos_celda;
	   if(in_array($x."-0",$_SESSION['datos']['celda'],true)){$valor=array_search($x."-0",$_SESSION['datos']['celda']);}
	  (in_array($x."-0",$_SESSION['datos']['celda'],true))? $datos_celda=$datos_celda="Materia: ".$_SESSION['datos']['nombre_materia'][$valor].'      Prof:'.$_SESSION['datos']['nombre'][$valor].' '.$_SESSION['datos']['apellido'][$valor].'             Aula: '.$_SESSION['datos']['nombre_ambiente'][$valor] :  $datos_celda='-';
