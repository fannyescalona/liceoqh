...
function NbLines($w,$txt)
{
  ...
  require_once("../clases/class_bd.php");
  $mysql=new Conexion();
  $sql="SELECT DISTINCT CONCAT(CASE UPPER(pa.nombre_pais) WHEN 'VENEZUELA' THEN 'V-' ELSE 'E-' END,pi.cedula) cedula,pi.cedula as ci,
p.nombres,p.apellidos,pa.nombre_pais,c.nombre_carrera carrera, 
CASE sec.turno WHEN 'M' THEN 'MAÑANA' ELSE 'TARDE' END turno,pri.disciplina
FROM tpersonas p 
...
inner join tinscrito_seccion isec on ps.cod_personal = isec.cod_personal 
inner join tseccion sec on isec.seccion = sec.seccion 
inner join tinscripcion i on pi.cod_inscripcion = i.cod_inscripcion 
inner join tperiodo per on i.cod_periodo = per.cod_periodo
WHERE per.fecha_desactivacion IS NULL AND i.fecha_desactivacion IS NULL";
  ...
  if($mysql->Total_Filas($data)!=0){
   ...
   //Table with 20 rows and 5 columns
     $lobjPdf->SetWidths(array(96,17,17,40,25));
         ...
         $lobjPdf->SetFont('Arial','B',8);
         $lobjPdf->Cell(47,$AL,utf8_decode($arr['cedula'][$i]),0,0);    
         $lobjPdf->Cell(70,$AL,utf8_decode($arr['nombres'][$i]),0,0);    
         $lobjPdf->Cell(47,$AL,utf8_decode($arr['apellidos'][$i]),0,0);    
         $lobjPdf->Cell(47,$AL,utf8_decode($arr['nombre_pais'][$i]),0,1);  
         ...
         $lobjPdf->SetFont('Arial','B',8);
         $lobjPdf->Cell(70,$AL,utf8_decode($arr['carrera'][$i]),0,0);    
         $lobjPdf->Cell(47,$AL,utf8_decode($arr['turno'][$i]),0,0);    
         $lobjPdf->Cell(47,$AL,utf8_decode($arr['disciplina'][$i]),0,1);    
          ...
          $sqlx="SELECT pi.cedula,m.cod_materia,INITCAP(LOWER(m.nombre_materia)) nombre_materia,sem.semestre,
        m.unidad_curricular,INITCAP(LOWER(sec.nombre_seccion)) seccion 
        FROM tproceso_inscripcion pi 
        inner join tpersonales ps on pi.cedula = ps.cedula 
        inner join tcarrera c on pi.cod_carrera = c.cod_carrera 
