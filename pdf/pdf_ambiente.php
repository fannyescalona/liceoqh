<?php
  ...
  $this->Cell(0,6,"UNIVERSIDAD DEPORTIVA DEL SUR",0,1,"C");
    $this->Cell(0,6,"\"Sistema de Ingreso Estudiantes y Horarios\"",0,1,"C");
       $this->Ln(15); 
   $this->Cell(0,6,'LISTADO DE LAS AULAS DE CLASES',0,1,"C");
   $this->Ln(5);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
                  //$lobjPdf->Cell($anchura,$altura,utf8_decode('N°'),1,0,'L',$color_fondo); 
                  $this->Cell($anchura*2,$altura,utf8_decode('CÓDIGO'),1,0,'C',$color_fondo); 
                  $this->Cell($anchura*4,$altura,'NOMBRE AULA',1,0,'C',$color_fondo); 
                  $this->Cell($anchura*4,$altura,'TIPO DE AULA',1,0,'C',$color_fondo); 
...
function NbLines($w,$txt)
{
  ...
  $sql="select *,
    ...
    ELSE 'Desactivado' END) AS estatus from tambiente";  
  $i=-1;
  $data=$mysql->Ejecutar($sql);
    if($mysql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
         utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['cod_ambiente'])))),
         utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['nombre_ambiente'])))),
         utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['tipo_ambiente'])))),
