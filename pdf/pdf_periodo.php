<?php
  ...
  $this->Cell(0,6,"UNIVERSIDAD DEPORTIVA DEL SUR",0,1,"C");
    $this->Cell(0,6,"\"Sistema de Ingreso Estudiantes y Horarios\"",0,1,"C");
       $this->Ln(15); 
   $this->Cell(0,6,utf8_decode('LISTADO DE LOS PERÍODOS'),0,1,"C");
   $this->Ln(5);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
                  ...
                  $this->Cell($anchura*6,$altura,utf8_decode('NOMBRE DEL PERÍODO'),1,0,'L',$color_fondo); 
                  $this->Cell($anchura*4,$altura,'FECHA INICIO',1,0,'L',$color_fondo); 
                  $this->Cell($anchura*4,$altura,'FECHA FIN',1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,60,40,40,26));
  ...
  $sql="select cod_periodo,descripcion,date_format(fecha_inicio,'%d/%m/%Y') fecha_inicio,date_format(fecha_fin,'%d/%m/%Y') fecha_fin,
    ...
    ELSE 'Desactivado' END) AS estatus from tperiodo WHERE esinscripcion = 'N'";
  $i=-1;
  $data=$mysql->Ejecutar($sql);
    if($mysql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
        utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['cod_periodo'])))),
        utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['descripcion'])))),
        utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['fecha_inicio'])))),
        utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['fecha_fin'])))),
