   ...
   //Cabecera de página
    public function Header()
    {
      ...
      $this->Cell(0,6,"\"Sistema de Ingreso Estudiantes y Contol de Notas\"",0,1,"C");
       $this->Ln(15); 
   $this->Cell(0,6,'LISTADO DE LOS PLANTELES',0,1,"C");
   $this->Ln(5);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
                  ...
                  $this->Cell($anchura*5,$altura,'NOMBRE DEL PLANTEL',1,0,'L',$color_fondo); 
                  $this->Cell($anchura*7,$altura,'DIRECCION',1,0,'L',$color_fondo);
                  $this->Cell($anchura*3,$altura,utf8_decode('TELEFÓNO'),1,0,'L',$color_fondo);
                  $this->Cell($anchura*3,$altura,'LOCALIDAD',1,0,'L',$color_fondo); 
                  $this->Cell($anchura*3,$altura,utf8_decode('MUNICIPIO'),1,0,'L',$color_fondo);
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,50,70,30,30,30,20));
  require_once("../clases/class_bd.php");
  $mysql=new Conexion();
    $sql="SELECT p.codigo_plantel codigo, p.nombre nombre, p.direccion direccion, p.telefono_habitacion tlf, p.localidad localidad, m.descripcion municipio,
  ...
  $data=$mysql->Ejecutar($sql);
    if($mysql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
        utf8_decode(ucwords($tperfil['codigo'])),
        utf8_decode(ucwords($tperfil['nombre'])),
        utf8_decode(ucwords($tperfil['direccion'])),
        utf8_decode(ucwords($tperfil['tlf'])),
        utf8_decode(ucwords($tperfil['localidad'])),
