<?php
  ...
  $this->Cell(0,6,"UNIVERSIDAD DEPORTIVA DEL SUR",0,1,"C");
    $this->Cell(0,6,"\"Sistema de Ingreso Estudiantes y Horarios\"",0,1,"C");
       $this->Ln(15); 
   $this->Cell(0,6,'LISTADO DE BLOQUE DE HORA',0,1,"C");
   $this->Ln(5);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
                  ...
                  $this->Cell($anchura*4,$altura,'HORA INICIO',1,0,'L',$color_fondo); 
                  $this->Cell($anchura*4,$altura,'HORA FIN',1,0,'L',$color_fondo); 
                  $this->Cell($anchura*4,$altura,utf8_decode('TURNO'),1,0,'L',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,40,40,40,26));
  require_once("../clases/class_bd.php");
  $mysql=new Conexion();
  $sql="select cod_bloque_hora,time_format(hora_inicio,'%h:%i %p') hora_inicio,
time_format(hora_fin,'%h:%i %p') hora_fin,Case turno When 'M' Then 'Mañana' Else 'Tarde' End turno,
    (CASE WHEN fecha_desactivacion IS NULL THEN  'Activo' 
    ELSE 'Desactivado' END) AS estatus from bloque_hora"; 
  $i=-1;
  $data=$mysql->Ejecutar($sql);
    if($mysql->Total_Filas($data)!=0){
         ...
         $lobjPdf->Row(array(
           utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['cod_bloque_hora'])))),
            utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['hora_inicio'])))),
            utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['hora_fin'])))),
            utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['turno'])))),
