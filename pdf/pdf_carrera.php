<?php
  ...
  $this->Cell(0,6,"UNIVERSIDAD DEPORTIVA DEL SUR",0,1,"C");
    $this->Cell(0,6,"\"Sistema de Ingreso Estudiantes y Horarios\"",0,1,"C");
       $this->Ln(15); 
   $this->Cell(0,6,'LISTADO DE LAS CARRERAS',0,1,"C");
   $this->Ln(5);
     ...
     $this->SetFillColor(0,0,140); 
         ...
         $this->SetTextColor(0,0,0);
                $this->Cell($avnzar);
                  ...
                  $this->Cell($anchura*7,$altura,('NOMBRE CARRERA'),1,0,'C',$color_fondo); 
...
function NbLines($w,$txt)
{
   ...
   //Table with 20 rows and 5 columns
      $lobjPdf->SetWidths(array(20,70,26));
 require_once("../clases/class_bd.php");
    $mysql=new Conexion();
    $sql="select *,
    (CASE WHEN fecha_desactivacion IS NULL THEN  'Activo' 
    ELSE 'Desactivado' END) AS estatus from tcarrera";
  $i=-1;
  $data=$mysql->Ejecutar($sql);
    if($mysql->Total_Filas($data)!=0){
        ...
        utf8_decode(ucwords(strtolower(@utf8_decode($tperfil['nombre_carrera'])))),
