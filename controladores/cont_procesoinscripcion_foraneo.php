...
if(isset($_POST['tiene_materia_pendiente']))
  $tiene_materia_pendiente = trim($_POST['tiene_materia_pendiente']);
...
if($operacion=='Registrar'){
  ...
  if(!$inscripcion->Comprobar()){
    if($inscripcion->Registrar()){
      if($tiene_materia_pendiente=="Y"){
        if($inscripcion->RegistrarMateriasPendientes($_POST['materias']))
          $confirmacion=1;
        else
          $confirmacion=-1;
      }
  ...
  if($confirmacion==1){
    ...
    header("Location: ../vistas/?proceso_inscripcion_foraneo#datospadres");
  }else{
    ...
    header("Location: ../vistas/?proceso_inscripcion_foraneo#datosestudiantes");
  }
}
...
if($operacion=="Paso2"){
  ...
  if($confirmacion==1){
    ...
    header("Location: ../vistas/?proceso_inscripcion_foraneo#documentosconsignados");
  }else{
    $inscripcion->Transaccion('cancelado');
    $_SESSION['datos']['cedula']=$inscripcion->cedula_estudiante();
...
if($operacion=="Paso3"){
  ...
  if($confirmacion==1){
    ...
    header("Location: ../vistas/?proceso_inscripcion_foraneo#datosrepresentante");
  }else{
    $inscripcion->Transaccion('cancelado');
    $_SESSION['datos']['cedula']=$inscripcion->cedula_estudiante();
...
if($operacion=="Paso4"){
  ...
  if($confirmacion==1){
    $inscripcion->Transaccion('finalizado');
    $_SESSION['datos']['estudiante_registrado']=$cedula_estudiante;
    $_SESSION['datos']['mensaje']="Los Datos del Representante han sido cargados con éxito.";
    header("Location: ../vistas/?proceso_inscripcion_foraneo");
  }else{
    $inscripcion->Transaccion('cancelado');
    $_SESSION['datos']['cedula']=$inscripcion->cedula_estudiante();
