...
valor=document.getElementById('nombre_ambiente').value;
valor2=document.getElementById('cod_ambiente').value;
valor3=document.getElementById('tipo_ambiente').value;
if(devuelve_boton(param)=="Registrar" || devuelve_boton(param)=="Modificar"){
	if(valor.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
	alert('Ingrese el nombre del aula')
	permitido=false;
	}	
	else if(valor2.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
	alert('Ingrese el código del aula')
	permitido=false;
	}	
	else if(valor3==0){ //para no permitir que se quede en blanco
	alert('Seleccione el tipo del aula')
	permitido=false;
	}
}
