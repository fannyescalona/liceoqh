...
function init(){
	...
	if($('#estudiante_registrado').val()!="")
		generateReport();

	function generateReport(){
		swal({
	          title: stringUnicode("Datos guardados con exito. ¿Quiere imprimir el formato de inscripción?"),
	          type: 'info',
	          ...
	          url = "../informe_pdf/ficha_inscripcion.php?cedula_estudiante="+$('#estudiante_registrado').val();
...
function validar_formulario(param){
	...
	valor15=document.getElementById('grado_escolar').value;
	valor16=$('input:radio[name=tiene_materia_pendiente]:checked').val();
	materias = document.getElementsByName('materias[]');
	//Utilizamos una expresion regular para validar email
	var regex = /[\w-\.]{2,}@([\w-]{2,}\.)*([\w-]{2,}\.)[\w-]{2,4}/;
	var cedula_rif = /^[VGJE-vgje]\d{8,9}$/;
	...
	if(devuelve_boton(param)=="Registrar" || devuelve_boton(param)=="Modificar"){
		...
		else if(valor1.replace(/^\s+|\s+$/gi,"").length!=0 && valor1.replace(/^\s+|\s+$/gi,"").length<=7){ //para no permitir que se quede en blanco
			alert('La cédula del estudiante ingresada debe ser mayor a 6 digitos')
			permitido=false;
		}
		...
		else if(valor2.replace(/^\s+|\s+$/gi,"").length!=0 && valor2.replace(/^\s+|\s+$/gi,"").length<=7){ //para no permitir que se quede en blanco
			alert('La cédula escolar ingresada debe ser mayor a 6 digitos')
			permitido=false;
		}
		...
		/*else if(valor11.replace(/^\s+|\s+$/gi,"").length==0){ //para no permitir que se quede en blanco
			...
			if(materias.length>0){
				...
				if(vacio>0){
					alert('¡Las materias no pueden quedar vacias!')
					permitido = false;
				}
			}
			else{
				alert('¡Debe agregar las materias pendientes!')
				permitido = false;
			}
		}
	}
...
function validar_formulario1(){
	permitido=true;
	ci_estudiante=document.getElementById('cedula').value;
	...
	if(valor0.replace(/^\s+|\s+$/gi,"").length!=0){
		...
		else if(valor0.replace(/^\s+|\s+$/gi,"").length<=7){
			alert('La cédula de la madre ingresada de ser mayor a 6 dígitos');
			permitido = false;
		}
		else if(ci_estudiante==valor0){
			alert('La cédula del estudiante: '+ci_estudiante+' no puede ser igual a la cédula de la madre: '+valor0);
			permitido = false;
		}
	...
	if(valor8.replace(/^\s+|\s+$/gi,"").length!=0){
		if(!cedula_rif.test(valor8.trim())){
			alert('Error en la cédula ingresada ('+valor8+'), formato permitido de una cédula/rif: V210569852 -> la letra puede ser V, G, J, E y minimo 7 maximo 9 digitos');
			permitido=false;
		}
		else if(valor8.replace(/^\s+|\s+$/gi,"").length<=7){
			alert('La cédula del padre ingresada de ser mayor a 6 dígitos');
			permitido = false;
		}
		else if(valor8.replace(/^\s+|\s+$/gi,"").length!=0 && ci_estudiante==valor8){
			alert('La cédula del estudiante: '+ci_estudiante+' no puede ser igual a la cédula del padre: '+valor8);
			permitido = false;
		}
...
function validar_formulario2(){
	permitido=true;
	valor0=document.getElementById('becadoY').checked==true && document.getElementById('becadoN').checked==false ? document.getElementById('becadoY').value : null;
...
function validar_formulario3(){
	...
	else if(valor1.replace(/^\s+|\s+$/gi,"").length<=7){
		alert('La cédula del representante ingresada de ser mayor a 6 dígitos')
		permitido=false;
	}
